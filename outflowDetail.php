...
$myQuery = mysql_query("SELECT T1.ID, T1.code, T2.name store, CONCAT(T3.first, ' ', T3.last) emp, DATE_FORMAT(T1.created_at, '%d-%m-%Y %T') created, T1.remarks FROM OUTFLOWS T1 INNER JOIN STORES T2 ON T1.storeID = T2.ID INNER JOIN CREW T3 ON T1.empID = T3.ID WHERE T1.ID = '$outID'");
    ...
    <tr>
      <td colspan="2">Partidas<br>
        <div id="itemContainer">
        	...
        	<?php
        	$myQuery = mysql_query("SELECT T2.ID, T2.code, T2.name, T1.qty FROM OUTLN T1 JOIN PRODUCT T2 ON T1.prodCode = T2.code WHERE T1.outID = '$outID'");
